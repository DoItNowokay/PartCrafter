...
def prepare_image(image_path, bg_color, rmbg_net, device="cuda"):
...
@torch.no_grad()
def validate_dataset(json_in, json_out, rmbg_net, device="cuda"):
    ...
    for entry in tqdm(data, desc="Validating images"):
    ...
    print(f"Done ✔  Invalid samples: {invalid}/{len(data)}")
...
if __name__ == "__main__":
    ...
    rmbg_net = BriaRMBG.from_pretrained(
        "pretrained_weights/RMBG-1.4"
    ).to(device)
    rmbg_net.eval()

    validate_dataset(
        json_in="/data1/simroop/DATA/PartCrafter/preprocessed_data/abo/object_part_configs.json",
        json_out="/data1/simroop/DATA/PartCrafter/preprocessed_data/abo/object_part_configs_valid.json",
        rmbg_net=rmbg_net,
        device=device,
    )
