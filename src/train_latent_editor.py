...
def main():
    ...
    parser.add_argument(
        ...
        type=str,
        default="none",
        choices=["none", "cross_attn_editing", "direct_tweak_latent", "l1_tweak_latent", "text_cross_attn"],
    ...
    while global_step < args.max_train_steps:
        for batch in train_loader:
            ...
            # image preprocessing
            with torch.no_grad():
                source_images_embed = feature_extractor(images=source_images, return_tensors="pt").pixel_values
                target_images_embed = feature_extractor(images=target_images, return_tensors="pt").pixel_values
            source_images_embed = source_images_embed.to(device=device, dtype=weight_dtype)
            target_images_embed = target_images_embed.to(device=device, dtype=weight_dtype)
            with torch.no_grad():
                dino_out = image_encoder(source_images_embed)
                source_image_embeds = dino_out.last_hidden_state
                source_image_pooled = dino_out.pooler_output
                target_image_embed = image_encoder(target_images_embed).last_hidden_state
            ...
            loss_cond_proc, _ = condition_processor(
                image=source_image_embeds,
                text=text_embeds,
                image_pooled=source_image_pooled,
